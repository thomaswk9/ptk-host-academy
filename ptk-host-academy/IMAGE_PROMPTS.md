...
Save the resulting PNGs into `public/images/`:
- `property-1-notting-hill.png`
- `property-2-shoreditch.png`
- `property-3-chelsea.png`
- `property-4-mayfair.png` *(you already have this — the upload from 30 Apr)*
- `property-5-kensington.png`

Wire them into the property cards in `index.html` next turn (or do it manually via `<img src="/images/property-1-notting-hill.png">` on each property card).
...
The game's property cards display the image at roughly 100% width, ~30% height of the card. So:
- 16:9 aspect ratio works well (1280×720 or 1920×1080)
- Or square (1024×1024) — the game CSS will crop top and bottom

PNG with transparent background not needed — solid background is fine.
